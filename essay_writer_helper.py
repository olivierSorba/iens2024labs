...
class writer_gui( ):
            ...
            def updt_disp():
                ...
                for state in self.graph.get_state_history(self.thread):
                    ...
                    try:
                        ...
                        s_checkpoint_id  = "no checkpoint_id"
    ...
    def launch(self,  server_port, share=False, server_name="0.0.0.0"):
        """Launches the interface on a local server.
        Args:
            server_port (int, None): e.g. 7860 The port number on which the server will run.
                                     If None let gradio decide, defaults to 7860.
            share (bool, optional): Whether the server is accessible from outside the local machine. Defaults to False.
            server_name (str, optional): The server name. Defaults to "0.0.0.0" to allow external access.
        """
        self.demo.launch(share=share, server_port= server_port, server_name=server_name)

if __name__ == '__main__':
    import sys
    sys.path.append('/home')
    from credentials import fix_credentials 
    print(f"found credentials ?: {fix_credentials()}")
    ew = ewriter()
    gui = writer_gui(ew.graph)
    gui.launch(server_port=None)